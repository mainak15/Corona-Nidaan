...
checkpointer = ModelCheckpoint(
    filepath=os.path.join('data', 'checkpoints', 'InceptionResNetV2_1.hdf5'),
    verbose=1,
    save_best_only=True)
...
timestamp = time.time()
csv_logger = CSVLogger(os.path.join('data', 'logs', 'InceptionResNetV2_1' + '-' + 'training-' + \
        str(timestamp) + '.log'))
...
def train_model(model, nb_epoch, generators, callbacks=[]):
    ...
    fig.savefig('InceptionResNetV2_1.png')
    return model
