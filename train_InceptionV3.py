...
checkpointer = ModelCheckpoint(
    filepath=os.path.join('data', 'checkpoints', 'InceptionV3_1.hdf5'),
    verbose=1,
    save_best_only=True)
...
timestamp = time.time()
csv_logger = CSVLogger(os.path.join('data', 'logs', 'InceptionV3_1' + '-' + 'training-' + \
        str(timestamp) + '.log'))
...
def main(weights_file):
    ...
    #model = freeze_all_but_mid_and_top(model)
    #model = train_model(model, 10000, generators,
